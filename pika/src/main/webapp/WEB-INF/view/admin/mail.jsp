...
<script type="text/javascript">
	function idinputchk(f) {
		if(f.naverid.value == "") {
			alert("네이버 아이디를 입력 하세요.");
			f.naverid.focus();
			return false;
		}
		if(f.naverpw.value == "") {
			alert("네이버 비밀번호를 입력 하세요.");
			f.naverpw.focus();
			return false;
		}
		return true;
...
<form name="mailform" method="post" action="mail"
	...
	<table class="w3-table">
		...
		<tr>
			<td>받는 사람</td>
			<td>
<input type="text" name="recipient" size="100" value=
'<c:forEach items="${list}" var="user">${user.username} &lt;${user.email}&gt;,
</c:forEach>' class="w3-input">
			</td>
		</tr>
		<tr>
			<td>제목</td>
