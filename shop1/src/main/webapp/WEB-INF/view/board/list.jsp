...
<table class="w3-table w3-border">
	...
	<form action="list" method="post" name="searchform">
		<td><input type="hidden" name="pageNum" value="1">
		...
		<td colspan="3"><input type="text" name="searchcontent" value="${param.searchcontent}" class="w3-input"></td>
	...
	<c:forEach var="board" items="${boardlist}">
		...
		<a href="detail?num=${board.num}">${board.title}</a></td>
			...
			<td><fmt:formatDate value="${board.regdate}" pattern="yyyyMMdd" var="rdate"/>
				...
				<c:if test="${today != rdate}">
					<fmt:formatDate value="${board.regdate}" pattern="YY-mm-dd HH:mm"/></c:if>
					<td>${board.readcnt}</td>
	</c:forEach>
	...
	<tr><td colspan="5">
		<a href="write" class="w3-right">[글쓰기]</a></td></tr>
</table>
</body>
</html>